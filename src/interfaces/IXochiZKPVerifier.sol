...
/// @dev Implementations route to per-proof-type UltraPlonk verifiers generated by Noir/Barretenberg
interface IXochiZKPVerifier {
    /// @notice Emitted when a proof is successfully verified
    /// @param proofType The type of proof verified (0x01-0x06)
    /// @param prover The address that submitted the proof
    /// @param publicInputsHash Keccak256 of the public inputs
    event ProofVerified(uint8 indexed proofType, address indexed prover, bytes32 publicInputsHash);

    /// @notice Verify a zero-knowledge compliance proof
    /// @param proofType The type of proof (0x01-0x06)
    /// @param proof The encoded UltraPlonk proof data
    ...
    /// @notice Get the address of the UltraPlonk verifier for a proof type
